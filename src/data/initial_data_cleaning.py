...
def process_restaurant_data(input_file, output_file):
    ...
    # 6. '(한시적)'이 포함된 음식점 제외
    df_filtered = df_cleaned[~df_cleaned['음식점명'].str.contains('(한시적)', regex=False)]
    
    # 7. 결과 확인 및 저장
    print(f"원본 데이터 행 수: {len(df_cleaned)}")
    print(f"필터링 후 데이터 행 수: {len(df_filtered)}")
    print(f"제거된 행 수: {len(df_cleaned) - len(df_filtered)}")
...
def main():
    ...
    default_input = os.path.join(script_dir, '..', '..', 'data', 'raw', '서울시 서초구 일반음식점 인허가 정보.csv')
    default_output = os.path.join(script_dir, '..', '..', 'data', 'interim', 'seocho_restaurants_cleaned.csv')
